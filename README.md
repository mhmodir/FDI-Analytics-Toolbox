...
Upload a CSV, walk a six-stage pipeline, and get fault windows, root-cause rankings, and AI-assisted interpretation — all in the browser.
...
**FDI Analytics** is a no-code condition monitoring toolbox for engineers and researchers working with industrial time-series data. It implements a complete Fault Detection and Identification (FDI) pipeline using only **unsupervised machine learning** — no historical fault labels, no annotations, and no separate training dataset required.

A short segment of normal operation serves as the baseline. Everything that deviates from it is scored, flagged, and ranked automatically.
...
> Currently at **v1.1**. See the [Roadmap](#roadmap--version-2) for planned improvements.  
...
[![Watch the demo](assets/FDIAd-gif.gif)](docs/FDI%20Ad.mp4)
...
| Home — Data Explorer | Pre-processing |
|:---:|:---:|
...
```
┌──────────────┐     ┌─────────────────┐     ┌───────────────────────┐
│  1. Home     │ →   │ 2. Pre-process  │ →   │ 3. Feature Extraction │
│  Data Upload │     │ Signal Cleaning │     │ Window-based Features │
└──────────────┘     └─────────────────┘     └───────────────────────┘
                                                           ↓
┌──────────────┐     ┌─────────────────┐     ┌───────────────────────┐
│  6. Report   │ ←   │ 5. Identify     │ ←   │ 4. Detect             │
│  Export & AI │     │ Root-Cause Rank │     │ Anomaly Scoring       │
└──────────────┘     └─────────────────┘     └───────────────────────┘
```
...
Five configurable conditioning stages applied in sequence:
...
Root-cause contribution analysis to pinpoint which variables and features drive each anomaly:
...
Built-in LLM assistant with full awareness of the current pipeline state — dataset shape, preprocessing applied, detection results, and top contributing features. Supports:
...
Pre-built quick prompts cover the most common tasks: interpret detection results, recommend preprocessing, compare methods, summarize findings, suggest next steps, and optimize window parameters.
...
1. **Home** — Upload your CSV/Excel file. Inspect raw signal plots. Set the baseline window using the range sliders.
2. **Pre-processing** — Enable and configure each conditioning step. Preview the before/after effect on your signals.
3. **Feature Extraction** — Choose variables, window parameters (size, step, sampling frequency), and features to compute.
4. **Detection** — Select a detection method, configure its hyperparameters, and run to get an anomaly score per window.
5. **Identification** — View a ranked heatmap of variable and feature contributions to understand which sensor is most anomalous.
...
- **Azure Container Registry (ACR)** was provisioned for image storage. ACR Tasks (`az acr build`) are restricted on the Azure for Students subscription tier; the final pipeline uses `az webapp up` with Azure's Oryx build system, which installs dependencies server-side from `requirements.txt` on each deployment.
...
- **Bug fixes** — known edge cases identified during v1 testing
...
- **Report & identification metrics** — expanded report section with more interpretable fault identification metrics, clearer visualisations, and actionable maintenance recommendations
